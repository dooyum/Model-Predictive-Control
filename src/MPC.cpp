...
class FG_eval {
 public:
  ...
  // `vars` is a vector containing the state & actuator values.
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 1; t++) {
      // Minimize steering angle error.
      fg[0] += 5 * CppAD::pow(vars[delta_start + t], 2);
      // Minimize acceleration error.
      fg[0] += 5 * CppAD::pow(vars[a_start + t], 2);
    }

    // Minimize the value gap between sequential actuations.
    for (int t = 0; t < N - 2; t++) {
      // Minimize steering jerking
      fg[0] += 200 * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      // Minimize stop and go acceleration
      fg[0] += 10 * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
    std::cout << "++++++ FG Cost " << fg[0] << std::endl;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = a_start; i < n_vars; i++) {
    vars_lowerbound[i] = -1; // Brake fully engaged
    vars_upperbound[i] = 1; // Gas pedal fully down
  }
